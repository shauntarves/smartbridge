...
class WyzeClient(object):
    ...
    def general_api_client(self):
        if not self._general_api_client:
            self._general_api_client = WyzeGeneralApiClient(self._config, self._access_token, self._user_id)
        return self._general_api_client

    @property
    def platform_client(self):
        if not self._platform_client:
            self._platform_client = WyzePlatformServiceClient(self._config, self._access_token)
        return self._platform_client

    @property
    def venus_client(self):
        if not self._venus_client:
            self._venus_client = WyzeVenusServiceClient(self._config, self._access_token)
    ...
    def list_vacuums(self):
      return [device for device in self.list_devices() if device['product_model'] in DeviceModels.VACUUM]
    
    def get_vacuum(self, device_mac, props, device_info_props):
        vacuums = [_vacuum for _vacuum in self.list_vacuums() if _vacuum['mac'] == device_mac]
        if len(vacuums) == 0:
          return None
        ...
        device_info_props = self.venus_client.get_device_info(device_mac, device_info_props)
        if (device_info_props and 'data' in device_info_props and device_info_props['data'] is not None):
          vacuum.update(device_info_props['data'])
        
        current_position = self.venus_client.get_current_position(device_mac)
        if (current_position and 'data' in current_position and current_position['data'] is not None):
          vacuum['current_position'] = current_position['data']
        
        current_map = self.venus_client.get_current_map(device_mac)
        if (current_map and 'data' in current_map and current_map['data'] is not None):
    ...
    def set_vacuum_mode(self, device_mac, device_model, type, value):
        self.venus_client.set_iot_action(device_mac, device_model, 'set_mode', {'type': type, 'value': value})
    
    def set_vacuum_preference(self, device_mac, device_model, control_type, value):
        self.venus_client.set_iot_action(device_mac, device_model, 'set_preference', {'ctrltype': control_type, 'value': value})
    ...
    def list_plugs(self):
      return [device for device in self.list_devices() if device['product_model'] in DeviceModels.PLUG]
    
    def get_plug(self, device_mac, props):
        plugs = [_plug for _plug in self.list_plugs() if _plug['mac'] == device_mac]
        if len(plugs) == 0:
          return None
        
        plug = plugs[0]
        plug.update(self.api_client.get_device_property_list(plug['mac'], plug['product_model'], props))
    ...
    def set_plug_property(self, device_mac, device_model, name, value):
        self.api_client.set_device_property(device_mac, device_model, name, value)
    
    def list_bulbs(self):
      return [device for device in self.list_devices() if device['product_model'] in DeviceModels.BULB]
    
    def get_bulb(self, device_mac, props):
        bulbs = [_bulb for _bulb in self.list_bulbs() if _bulb['mac'] == device_mac]
        if len(bulbs) == 0:
          return None
        
        bulb = bulbs[0]
        bulb.update(self.api_client.get_device_property_list(bulb['mac'], bulb['product_model'], props))
...
class WyzeServiceClient(object):
    ...
    def _do_request(self, session: requests.Session, request:requests.Request):
      try:
        log.trace('request')
        ...
        log.debug('sending ' + request.method + ' request to ' + request.url)
        ...
        if 'code' in response_json:
          ...
          if response_code != '1' and 'msg' in response_json and response_json['msg'] == 'AccessTokenError':
            log.warning("The user account is locked. Please resolve this issue and try again.")
            raise ProviderConnectionException("Failed to login with response: {0}".format(response_json))
          if response_code != '1' and 'msg' in response_json and response_json['msg'] == "UserIsLocked":
            log.warning("The user account is locked. Please resolve this issue and try again.")
            raise ProviderConnectionException("Failed to login with response: {0}".format(response_json))
          if response_code != '1' and 'msg' in response_json and response_json['msg'] == "UserNameOrPasswordError":
            log.warning("The username or password is incorrect. Please check your credentials and try again.")
            raise ProviderConnectionException("Failed to login with response: {0}".format(response_json))
          if response_code == '1001':
            log.error("Request to: {} does not respond to parameters in payload {} and gave a result of {}".format(request.url, request.body, response_json))
            raise ProviderInternalException("Parameters passed to Wyze Service do not fit the endpoint")
          if response_code == '1003':
            # FIXME what do I mean?
            log.error("Request to: {} does not respond to parameters in payload {} and gave a result of {}".format(request.url, request.body, response_json))
            raise ProviderInternalException("Parameters passed to Wyze Service do not fit the endpoint")
          if response_code == '1004':
            log.error("Request to: {} does not have the correct signature2 of {} and gave a result of {}".format(request.url, request.headers.signature2, response_json))
            raise ProviderInternalException("Parameters passed to Wyze Service do not fit the endpoint")
          if response_code != '1':
            print(type(response_code))
            log.error("Request to: {} failed with payload: {} with result of {}".format(request.url, request.body, response_json))
            raise ProviderInternalException("Failed to connect to the Wyze Service")
    ...
    def do_post(self, url: str, headers: dict, payload: dict):
      with self.session as client:
        if headers is not None:
          ...
          log.trace('merging request-specific headers into session headers')
...
class WyzeWpkNetServiceClient(WyzeServiceClient):
    ...
    def get_from_server(self, url, payload={}):
      ...
      headers = {
        ...
        'signature2': self.dynamic_signature(self.get_sorted_params(sorted(payload.items()))),
      }
    ...
    def dynamic_signature(self, message=''):
      _signing_key = md5_string((self._access_token if self._access_token is not None else '') + WyzeWpkNetServiceClient.SALTS[self.app_id])

      return hmac.new(_signing_key.encode('utf-8'), msg=message.encode('utf-8'), digestmod=md5).hexdigest()
...
class WyzeVenusServiceClient(WyzeSignatureServiceClient):
    ...
    def get_current_position(self, did):
      return self.get_from_server(self.endpoint_url + '/plugin/venus/memory_map/current_position', payload = {
          'did': did,
      })
    
    def get_current_map(self, did):
      return self.get_from_server(self.endpoint_url + '/plugin/venus/memory_map/current_map', payload = {
          'did': did,
      })
    
    def get_sweep_records(self, did, keys):
      return self.get_from_server(self.endpoint_url + '/plugin/venus/sweep_record/query_data', payload = {
          'purpose': 'history_map',
    ...
    def get_iot_prop(self, did, keys):
      return self.get_from_server(self.endpoint_url + '/plugin/venus/get_iot_prop', payload = {
          'did': did,
          'keys': ','.join(keys),
      })
    
    def get_device_info(self, did, keys):
      return self.get_from_server(self.endpoint_url + '/plugin/venus/device_info', payload = {
          'device_id': did,
          'keys': ','.join(keys),
      })
    
    def set_iot_action(self, did, model, cmd, params, is_sub_device=False):
      return self.post_to_server(self.endpoint_url + '/plugin/venus/set_iot_action', payload = {
          'cmd': cmd,
          'did': did,
          'model': model,
...
class WyzePlatformServiceClient(WyzeExServiceClient):
    ...
    def get_variable(self, keys):
      return self.get_from_server(self.endpoint_url + '/app/v2/platform/get_variable', payload = {
          'keys': ','.join(keys),
          'category': 'app',
        })
    
    def get_user_profile(self):
      return self.get_from_server(self.endpoint_url + '/app/v2/platform/get_user_profile')
...
class WyzeAuthServiceClient(WyzeExServiceClient):
    ...
    def login(self, email='', password=''):
      ...
      with self.session as client:
        ...
        return self.do_post(self.endpoint_url + '/user/login', None, payload)
...
class WyzeGeneralApiClient(WyzeServiceClient):
    ...
    def post_user_event(self, pid, event_id, event_type):
      ...
      return self.post_to_server(self.endpoint_url + '/v1/user/event', '', payload)
...
class WyzeApiClient(WyzeServiceClient):
    ...
    def refresh_token(self):
      ...
      return self.post_to_server(self.endpoint_url + '/app/user/refresh_token', {'refresh_token': self._refresh_token, 'sv': SV_REFRESH_TOKEN})
    
    def set_device_property(self, mac, model, pid, value):
      SV_SET_DEVICE_PROPERTY = '44b6d5640c4d4978baba65c8ab9a6d6e'
      
      return self.post_to_server(self.endpoint_url + '/app/v2/device/set_property', {'device_mac': mac, 'device_model': model, 'pid': pid, 'pvalue': value, 'sv': SV_SET_DEVICE_PROPERTY})
    ...
    def get_device_list_property_list(self, devices=[], target_pids=[]):
      ...
      return self.post_to_server(self.endpoint_url + '/app/v2/device_list/get_property_list', {'device_list': devices, 'target_pid_list': target_pids, 'sv': SV_GET_DEVICE_LIST_PROPERTY_LIST})

    def get_device_property_list(self, mac, model, target_pids=[]):
      SV_GET_DEVICE_PROPERTY_LIST = '1df2807c63254e16a06213323fe8dec8'
      
      return self.post_to_server(self.endpoint_url + '/app/v2/device/get_property_list', {'device_mac': mac, 'device_model': model, 'target_pid_list': target_pids, 'sv': SV_GET_DEVICE_PROPERTY_LIST})

    def get_object_list(self):
      SV_GET_DEVICE_LIST = 'c417b62d72ee44bf933054bdca183e77'

      return self.post_to_server(self.endpoint_url + '/app/v2/home_page/get_object_list', {'sv': SV_GET_DEVICE_LIST})