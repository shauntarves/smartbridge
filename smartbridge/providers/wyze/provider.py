...
class WyzeProvider(BaseProvider):
    ...
    def __init__(self, config):
        ...
        self.app_version = self._get_config_value('wyze_app_version', '2.16.55')
        
        self.phone_id = self._get_config_value('wyze_phone_id', str(uuid.uuid4()))
        self.phone_system_type = self._get_config_value('wyze_phone_system_type', 2)
        
        self.session_cfg = {
            'wyze_username': self._get_config_value('wyze_username', get_env('WYZE_USERNAME')),
            'wyze_password': self._get_config_value('wyze_password', get_env('WYZE_PASSWORD')),
            'wyze_access_token': self._get_config_value('wyze_access_token', None)
        }
