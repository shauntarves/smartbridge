...
class ProviderFactory(object):
    """
    Get info and handle on the available provider implementations.
    """
    ...
    def register_provider_class(self, cls):
        ...
        inherit from smartbridge.interfaces.Provider
        and also have a class attribute named PROVIDER_ID.
        The PROVIDER_ID is a user friendly name for the provider,
        such as 'smartbridgecam.com'. The PROVIDER_ID must also be included
        in the smartbridge.factory.ProviderList.
        :type  cls: class
        :param cls: A class implementing the Provider interface.
        """
        if isinstance(cls, type) and issubclass(cls, Provider):
            if hasattr(cls, "PROVIDER_ID"):
                ...
                log.warning("Provider class: %s implements Provider but"
                            " does not define PROVIDER_ID. Ignoring...", cls)
        else:
            log.debug("Class: %s does not implement the Provider"
                      "  interface. Ignoring...", cls)

    def discover_providers(self):
        """
        Discover all available providers within the
        ``smartbridge.providers`` package.
    ...
    def list_providers(self):
        ...
        submodules in smartbridge.providers.
        :rtype: dict
        :return: A dict of available providers and their implementations in the
                 following format::
                 {'aws': {'class': aws.provider.AWSProvider},
    ...
    def create_provider(self, name, config):
        """
        Searches all available providers for a Provider interface with the
        ...
        provider.
        :type name: str
        :param name: Provider name: one of ``smartbridgecam.com``.
        ...
        :rtype: ``object`` of :class:`.Provider`
        """
